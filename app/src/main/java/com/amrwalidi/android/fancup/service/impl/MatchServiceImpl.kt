...
class MatchServiceImpl @Inject constructor() : MatchService {
    ...
    override suspend fun createInvitedMatch(id: String, opponentId: String): Flow<Response> =
        callbackFlow {
            val job = launch {
                val existingMatchSnapshot = try {
                    matchRef.whereEqualTo("active", true).get().await()
                ...
                if (existingMatch != null) {
                    ...
                    try {
                        matchRef.document(existingMatch.id).update("players", updatedPlayers)
                            .await()
                        trySend(Response.Success(existingMatch.id))
                    } catch (e: Exception) {
                        trySend(Response.Failure(e))
                    }
                    close()
                    ...
                    val match = mapOf(
                        "players" to listOf(
                            mapOf("id" to id, "ready" to true),
                            mapOf("id" to opponentId, "ready" to false)
                        ),
                        "created_at" to FieldValue.serverTimestamp(),
                        "active" to true,
                        "question" to question,
                    ...
                    try {
                        val matchDoc = matchRef.add(match).await()
                        trySend(Response.Success(matchDoc.id))
                    } catch (e: Exception) {
                        trySend(Response.Failure(e))
                    }
                    close()
